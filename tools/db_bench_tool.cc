...
/////for fillrandomcontrolrequest
  port::Mutex queue_mu[REQUEST_QUEUE];   //mutex of req_op_queues
  std::queue<uint64_t> req_op_queues[REQUEST_QUEUE];  // request queue, save the time to join the queue
    ...
    if( thread->tid == 0 ) {  //record latency and throughput per second
      ...
      while(true) {
        ...
        RECORD_INFO(1,"now=,%.2f,s speed=,%.2f,MB/s,%.1f,iops size=,%.1f,MB average=,%.2f,MB/s,%.1f,iops request_num,%lu,queue_size,%lu,\n",
                    ...
                    thread->shared->req_op_queues[0].size());
      ...
      while(done < num) {
        ...
        if(thread->shared->req_op_queues[queue_index].size() < FLAGS_per_queue_length ) {
          thread->shared->req_op_queues[queue_index].push(FLAGS_env->NowMicros());
          push_ok = true;
        }
      ...
      while(done < num) {
        ...
        if(!thread->shared->req_op_queues[thread->tid - 2 ].empty()){
          uint64_t add_to_queue_micro = thread->shared->req_op_queues[thread->tid - 2 ].front();
          thread->shared->req_op_queues[thread->tid - 2 ].pop();
